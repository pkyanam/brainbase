  ...
  return (
    <div className="min-h-screen bg-bb-bg-primary text-bb-text-primary">
      ...
      <div className="max-w-2xl mx-auto px-6 py-12">
        ...
        <p className="text-bb-text-muted text-sm mb-8">Manage your brain and API keys.</p>
        ...
        <section className="mb-12">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={createKey}
              disabled={loading}
              className="px-4 py-2 bg-bb-accent hover:bg-bb-accent-dim disabled:opacity-50 text-bb-bg-primary text-sm font-medium rounded-lg transition-colors"
            >
              ...
              {keys.map((k) => (
                <div key={k.id} className="flex items-center justify-between p-4 border border-bb-border rounded-xl bg-bb-bg-secondary hover:border-bb-border-hover transition-colors">
                  <div>
                    ...
                    <div className="text-xs text-bb-text-muted font-mono mt-0.5">{k.key_prefix}...··· · Created {new Date(k.created_at).toLocaleDateString()}</div>
                  </div>
                  <button
          ...
          <div className="p-4 border border-bb-border rounded-xl bg-bb-bg-secondary">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <code className="text-bb-text-secondary font-mono text-xs">{user.id}</code>
              </div>
              <div>
                <span className="text-bb-text-muted block text-xs uppercase tracking-wide mb-1">Email</span>
                <span className="text-bb-text-secondary">{user.primaryEmailAddress?.emailAddress || "N/A"}</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="bg-bb-bg-secondary border border-bb-border rounded-xl p-5 space-y-3">
            <div>
              ...
              <code className="block text-sm text-bb-text-secondary font-mono mt-1">{baseUrl}</code>
            </div>
            <div>
              <span className="text-xs text-bb-text-muted uppercase tracking-wide">Auth header</span>
              <code className="block text-sm text-bb-text-secondary font-mono mt-1">Authorization: Bearer bb_liv...</code>
            </div>
            <div>
