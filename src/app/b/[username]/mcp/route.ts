...
const tools = [
  ...
  { name: "traverse_graph", description: "Traverse the knowledge graph with optional type filtering", inputSchema: { type: "object", properties: { slug: { type: "string" }, depth: { type: "number" }, direction: { type: "string", enum: ["out", "in", "both"] }, link_type: { type: "string", description: "Filter edges by link type (e.g. works_at, invested_in)" } }, required: ["slug"] } },
...
export async function POST(
  ...
  if (!token) {
    return NextResponse.json(
      { jsonrpc: "2.0", id: null, error: { code: -32001, message: "Missing Authorization header. Use: Bearer bb_live_..." } },
      { status: 401 }
    );
  }
  ...
  if (!keyData) {
    return NextResponse.json(
      { jsonrpc: "2.0", id: null, error: { code: -32001, message: "Invalid or revoked API key" } },
      { status: 401 }
    );
  }
  ...
  try {
    const { method, params: bodyParams } = body;
    ...
    if (method === "initialize") {
      return NextResponse.json({
        ...
        result: {
          ...
          capabilities: { tools: {} },
          serverInfo: {
            name: `Brainbase — ${username}'s brain`,
            version: "0.3.0",
          },
        },
      });
    ...
    if (method === "tools/call") {
      const toolParams = bodyParams as { name?: string; arguments?: Record<string, unknown> };
    ...
    return NextResponse.json(
      { jsonrpc: "2.0", id: body.id, error: { code: -32601, message: `Method not found: ${method}` } },
      { status: 404 }
    );
  } catch (err) {
    return NextResponse.json({
